...
class CosmicLaserShowDb(BaseDataStore):
    def __init__(self, path):
        ...
        if not existed:
            ...
            c = conn.cursor()
            c.execute('''CREATE TABLE Hits(GPSTime int, ID int, Timing int)''')
            conn.commit()
            conn.close()
    ...
    def store_event(self, event):
        ...
        module_id = 0
        print("Storing event {}".format(event))
        conn = sqlite3.connect(self.path)
        c = conn.cursor()
        c.execute('''INSERT INTO Hits VALUES (?, ?, ?)''', event.timestamp, 0 + module_id, event.t1)
        c.execute('''INSERT INTO Hits VALUES (?, ?, ?)''', event.timestamp, 1 + module_id, event.t2)
        conn.commit()
        conn.close()
